...
def get_report(report_id):
    try:
        ...
        # Convert report data to JSON-safe format
        safe_report = convert_to_native_types(report)
        
        # Add CORS headers
        response = jsonify(safe_report)
...
def get_analysis_prompt(context, analysis_data):
    """Generate context-specific analysis prompt"""
    base_prompt = f"""
    As a data analyst specializing in {context}, analyze this dataset and provide expert insights:
    
    Dataset Context: {context.title()}
    Records: {analysis_data['records']}
    Columns: {analysis_data['columns']}
    Data Types: {json.dumps(analysis_data['data_types'])}
    
    Provide analysis in the following structure:
    1. Key Findings:
       - List 3-5 major patterns or insights
       - Include statistical significance where relevant
       - Explain relationships between metrics
    
    2. Domain-Specific Analysis:
       - Focus on {context}-specific metrics
    ...
    {{
        "key_findings": [
            {{
                ...
                "confidence": 0.0-1.0,
                "statistical_context": "Statistical explanation",
                "relationships": ["Related metrics"]
            }}
        ],
        "domain_analysis": {{
            "metrics": [
...
def analyze_data():
    """Analyze data with enhanced context-aware processing"""
    try:
        report = request.json
        context = report.get('context_type', 'general')
        ...
        # Get context-specific analysis prompt
        analysis_prompt = get_analysis_prompt(context, analysis_data)
        ...
        if response.status_code == 200:
            ...
            # Format findings as readable HTML with statistical context
            formatted_analysis = format_analysis_for_display(analysis)
            
            return jsonify({
                "analysis": analysis,
                "formatted_html": formatted_analysis,
...
def format_analysis_for_display(analysis):
    """Convert JSON analysis to formatted HTML with statistical context"""
    ...
    for finding in analysis.get('key_findings', []):
        ...
        html += f"""
            <div class='finding'>
                ...
                <p class='statistical-context'>{finding.get('statistical_context', '')}</p>
                <p class='relationships'>Related metrics: {', '.join(finding.get('relationships', []))}</p>
            </div>
        """
    html += "</section>"
    
    # Domain Analysis
    html += "<section class='domain-analysis'>"
    html += "<h2>Domain-Specific Analysis</h2>"
    for metric in analysis.get('domain_analysis', {}).get('metrics', []):
        html += f"""
            <div class='metric'>
                <h3>{metric['name']}</h3>
                <p class='value'>{metric['value']}</p>
                <p class='interpretation'>{metric['interpretation']}</p>
                <p class='benchmark'>Benchmark: {metric['benchmark']}</p>
            </div>
        """
    html += "</section>"
